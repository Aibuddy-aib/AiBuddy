...
// Intelligently assign players to worlds (only during login)
// export const assignPlayerToWorld = mutation({
//   args: {
//     ethAddress: v.string(),
//     name: v.string(),
//   },
...
//     message: string;
//   }> => {
//     // 1. Check if player already exists in any world
...
//     if (existingPlayer) {
//       // Player already exists, return existing information
//       return {
//         success: true,
...
//     // 2. Find the best world (least players and not full)
//     const bestWorld = await ctx.runQuery(api.worldManager.findBestWorld, {});
    
//     let targetWorldId: Id<'worlds'>;
    
//     if (!bestWorld) {
//       // No available worlds, create new world
//       const newWorld = await ctx.runMutation(api.worldManager.createNewWorld, {});
//       targetWorldId = newWorld.worldId;
//     } else {
//       targetWorldId = bestWorld.worldId;
//     }
    
//     // 3. Register player in new world or selected world
//     const now = Date.now();
    
//     // Generate random avatar
//     const avatarNumber = Math.floor(Math.random() * 7) + 1;
//     const finalAvatarNumber = avatarNumber === 2 ? 8 : avatarNumber;
//     const avatarPath = `/assets/f${finalAvatarNumber}.png`;
    
//     // Generate playerId
//     const world = await ctx.db.get(targetWorldId as Id<'worlds'>);
//     if (!world) {
//       throw new ConvexError(`World ${targetWorldId} not found`);
//     }
    
//     const playerId = `aib-${world.nextId}`;
...
//     // Join world through game engine
//     await insertInput(ctx, targetWorldId, 'join', {
//       name: args.name,
//       character: (() => {
//         const match = avatarPath.match(/f(\d+)\.png/);
...
//     console.log(`Assigned player ${args.name} to world ${targetWorldId} (login-time allocation)`);
    
//     return {
//       success: true,
//       worldId: targetWorldId,
//       playerId: playerId,
//       message: 'Player assigned to world during login'
//     };
//   }
// });
